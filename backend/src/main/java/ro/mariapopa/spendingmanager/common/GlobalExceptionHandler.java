...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiError> handleNotFound(ResourceNotFoundException ex) {
    ApiError error =
        new ApiError(Instant.now(), HttpStatus.NOT_FOUND.value(), ex.getMessage(), null);
  ...
  public ResponseEntity<ApiError> handleStatementParse(StatementParseException ex) {
    ApiError error =
        new ApiError(Instant.now(), HttpStatus.BAD_REQUEST.value(), ex.getMessage(), null);
    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
  }
}
